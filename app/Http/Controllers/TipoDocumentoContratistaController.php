...
use App\Models\TipoDocumentoContratista;
use Illuminate\Http\Request;
use Illuminate\Database\QueryException;
use Illuminate\Database\Eloquent\ModelNotFoundException;

class TipoDocumentoContratistaController extends Controller
{
    ...
    {   
        $result = TipoDocumentoContratista::where('eliminado', 0)->get();
        if (count($result) > 0) {
    ...
    {
        try{
            $result = TipoDocumentoContratista::where('eliminado', 0)->where('id', $id)->first();
            return response()-> json(
                array('data' => $result, 'message' => config('constants.messages.3.message')),
                config('constants.messages.3.code'));
        }catch (Exception $ex){
    ...
    public function insertar(Request $request)
    {
        ...
        try {
            $areas = TipoDocumentoContratista::insert($data);
            return response() -> json(
    ...
    public function actualizar(Request $request)
    {
        ...
        try {
            TipoDocumentoContratista::findOrFail($id) -> update($data);
            return response() -> json(
